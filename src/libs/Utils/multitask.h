...
class TOTEM_UTILS_EXPORT MultiTaskBase : public QObject, public QRunnable
{
    Q_OBJECT
protected slots:
...
{
public:
    MultiTask(void (Class::*fn)(QFutureInterface<R> &),
              const QList<Class *> &objects)
        : fn(fn),
        objects(objects)
    {
    ...
    void run()
    {
        ...
        foreach (Class *object, objects)
        {
            ...
            connect(watcher, SIGNAL(progressTextChanged(QString)),
