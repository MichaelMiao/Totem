...
QVariant ItemProxyModel::data( const QModelIndex & index, int role /*= Qt::DisplayRole */ ) const
{
	if (index.isValid())
	{
		if(role == Qt::DisplayRole)
		{
			...
			return strTemp.split(Constants::WIZARD_SEPARATOR).back();
		}
...
void WizardModel::addItem( QStandardItem *topLevelCategoryItem, IWizard *wizard )
{
	...
	QStringList categroyHierarchy = categoryName.split(Constants::WIZARD_SEPARATOR);
...
static inline QString fixCategory(const QString &category)
{
	...
	if(category.length() == 0)
		return QString();
	if (ret.at(ret.length() - 1) != Constants::WIZARD_SEPARATOR)
	{
		ret += Constants::WIZARD_SEPARATOR;
	}
	return ret;
}
QList<IWizard*> WizardModel::wizards( const QString &category )
{
	...
	QStringList categroyHierarchy = fixedCategory.split(Constants::WIZARD_SEPARATOR);
