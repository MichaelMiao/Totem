...
/*!
    ...
    \brief 提示用户需要覆盖的文件列表，并可以使用复选框选择
*/

/**
 * \fn	static inline QString fileNameOfItem(const QStandardItem *item)
 *
 * \brief	返回存储在QStandardItem中的FileName,存放在FileNameRole中。
 *
 *
 * \param	item	The item.
 *
    ...
    m_model(new QStandardItemModel(0, 1, this))
{
    ...
    setModal(true);	/// 模态对话框
    setWindowFlags(windowFlags() & ~Qt::WindowContextHelpButtonHint);/// 取消掉帮助按钮
